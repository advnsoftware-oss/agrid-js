...
export type AgridStateLogger = (title: string, stateEvent: StateEvent) => void

export interface AgridStateLoggerConfig<S = any> {
     ...
     * normally this is only changed with agrid support assistance
     */
...
// we need a agrid logger for the rate limiter
const phConsoleLogger: Logger = createLogger('[Agrid Action RateLimiting]')

export function browserConsoleLogger(title: string, stateEvent: StateEvent): void {
    // but the agrid logger swallows messages unless debug is on
...
/**
 * Logger that sends state events to Agrid session recordings
 * Requires that the loaded agrid instance is provided
 * And returns the function to use as the logger
 *
 * e.g. const config = { logger: sessionRecordingLoggerForAgridInstance(agrid) }
 */
export const sessionRecordingLoggerForAgridInstance: (agrid: Agrid) => AgridStateLogger =
    (postHogInstance: Agrid) =>
 ...
 */
export function agridKeaLogger<S = any>(config: AgridStateLoggerConfig<S> = {}) {
    const middleware = agridReduxLogger(config)

    return {
        name: 'agrid-kea-logger',
        events: {
 ...
 */
export function agridReduxLogger<S = any>(
    config: AgridStateLoggerConfig<S> = {}
