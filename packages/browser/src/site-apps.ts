...
export class SiteApps {
    ...
    constructor(private _instance: Agrid) {
        // events captured between loading agrid-js and the site app; up to 1000 events
    ...
    get siteAppLoaders(): SiteAppLoader[] | undefined {
        return assignableWindow._AGRID_REMOTE_CONFIG?.[this._instance.config.token]?.siteApps
    }
    ...
    setupSiteApp(loader: SiteAppLoader) {
        ...
        try {
            ...
            logger.error(`Error while initializing Agrid app with config id ${loader.id}`, e)
            onLoaded(false)
        }
        ...
        if (hasInitReturned && app.loaded) {
            try {
                ...
                logger.error(`Error while processing buffered events Agrid app with config id ${loader.id}`, e)
                app.errored = true
            }
        }
    }
    ...
    onRemoteConfig(response: RemoteConfig): void {
        if (this.siteAppLoaders?.length) {
            if (!this.isEnabled) {
                logger.error(`Agrid site apps are disabled. Enable the "opt_in_site_apps" config to proceed.`)
                return
            }
        ...
        for (const { id, url } of response['siteApps']) {
            ...
            assignableWindow.__AgridExtensions__?.loadSiteApp?.(this._instance, url, (err) => {
                if (err) {
                    return logger.error(`Error while initializing Agrid app with config id ${id}`, err)
                }
            })
        }
    }
