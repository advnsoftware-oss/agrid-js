...
import { defaultConfig, Agrid } from '../agrid-core'
...
describe('AgridExceptions', () => {
    const captureMock = jest.fn()
    let agrid: Agrid
    let exceptions: AgridExceptions
    ...
    beforeEach(() => {
        ...
        exceptions = new AgridExceptions(agrid)
    })
    ...
    describe('sendExceptionEvent', () => {
        ...
        describe('Agrid SDK exceptions', () => {
            const inAppFrame = {
                filename: '../src/in-app-file.js',
                platform: 'javascript:web',
            }
            const agridFrame = {
                filename: 'https://internal-t.agrid.com/static/array.js',
                platform: 'javascript:web',
            }

            it('does not capture exceptions thrown by the Agrid SDK', () => {
                const exception = { stacktrace: { frames: [inAppFrame, agridFrame], type: 'raw' } }
                exceptions.sendExceptionEvent({ $exception_list: [exception] })
                expect(captureMock).not.toBeCalledWith(
                    '$exception',
                    { $exception_list: [exception] },
            ...
            it('captures the exception if a frame from the Agrid SDK is not the kaboom frame', () => {
                const exception = { stacktrace: { frames: [agridFrame, inAppFrame], type: 'raw' } }
                exceptions.sendExceptionEvent({ $exception_list: [exception] })
                expect(captureMock).toBeCalledWith('$exception', { $exception_list: [exception] }, expect.anything())
            })

            it('captures exceptions thrown within the Agrid SDK when enabled', () => {
                config.error_tracking.__captureAgridExceptions = true
                const exception = { stacktrace: { frames: [inAppFrame, agridFrame], type: 'raw' } }
                exceptions.sendExceptionEvent({ $exception_list: [exception] })
                expect(captureMock).toBeCalledWith('$exception', { $exception_list: [exception] }, expect.anything())
            })
