...
function makeAgridConfig(name: string, persistenceMode: string): AgridConfig {
    return <AgridConfig>{
        name,
...
describe('persistence', () => {
    ...
    describe.each(persistenceModes)('persistence modes: %p', (persistenceMode) => {
        ...
        beforeEach(() => {
            library = new AgridPersistence(makeAgridConfig('test', persistenceMode))
            library.clear()
        })
        ...
        it('should register_once', () => {
            ...
            const lib2 = new AgridPersistence(makeAgridConfig('test', persistenceMode))
            expect(lib2.props).toEqual({ distinct_id: 'hi', test_prop: 'test_val' })
        })

        it('should save user state', () => {
            const lib = new AgridPersistence(makeAgridConfig('bla', persistenceMode))
        ...
        it(`should only call save if props changes`, () => {
            const lib = new AgridPersistence(makeAgridConfig('test', 'localStorage+cookie'))
    ...
    describe('localStorage+cookie', () => {
        ...
        it('should migrate data from cookies to localStorage', () => {
            const lib = new AgridPersistence(makeAgridConfig('bla', 'cookie'))
            lib.register_once({ distinct_id: 'testy', test_prop: 'test_value' }, undefined, undefined)
            expect(document.cookie).toContain(
                'ph__agrid=%7B%22distinct_id%22%3A%22testy%22%2C%22test_prop%22%3A%22test_value%22%7D'
            )
            const lib2 = new AgridPersistence(makeAgridConfig('bla', 'localStorage+cookie'))
            expect(document.cookie).toContain('ph__agrid=%7B%22distinct_id%22%3A%22testy%22%7D')
            lib2.register({ test_prop2: 'test_val', distinct_id: 'test2' })
            expect(document.cookie).toContain('ph__agrid=%7B%22distinct_id%22%3A%22test2%22%7D')
            expect(lib2.props).toEqual({ distinct_id: 'test2', test_prop: 'test_value', test_prop2: 'test_val' })
            lib2.remove()
            expect(localStorage.getItem('ph__agrid')).toEqual(null)
        ...
        it(`should additionally store certain values in cookies if localStorage+cookie`, () => {
            ...
            const newLib = new AgridPersistence(makeAgridConfig('test', 'localStorage+cookie'))
        ...
        it('should allow swapping between storage methods', () => {
            ...
            let config = makeAgridConfig('test', 'localStorage+cookie')
            const lib = new AgridPersistence(makeAgridConfig('test', 'localStorage+cookie'))
            ...
            expect(document.cookie).not.toContain('test_prop')
            expect(localStorage.getItem('ph__agrid')).toEqual(JSON.stringify(expectedProps()))

            // Swap to memory
            let newConfig = makeAgridConfig('test', 'memory')
            ...
            newConfig = makeAgridConfig('test', 'localStorage')
            lib.update_config(newConfig, config)
            config = newConfig
    ...
    describe('agrid', () => {
        it('should not store anything in localstorage, or cookies when in sessionStorage mode', () => {
            const token = uuidv7()
            const persistenceKey = `ph_${token}_agrid`
            const agrid = new Agrid().init(token, {
                persistence: 'sessionStorage',
            })
            agrid.register({ distinct_id: 'test', test_prop: 'test_val' })
...
describe('agrid instance persistence', () => {
    beforeEach(() => {
        resetSessionStorageSupported()
        resetLocalStorageSupported()
    })
