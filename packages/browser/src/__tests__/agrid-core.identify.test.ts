    ...
    describe('invalid id passed', () => {
        it('does not update user', () => {
            ...
            expect(console.error).toHaveBeenCalledWith(
                '[Agrid.js]',
                'Unique user id has not been set in agrid.identify'
            )
        })

        it('does not update user when distinct ID is $agrid_cookieless', () => {
            console.error = jest.fn()

            instance.debug()

            instance.identify('$agrid_cookieless')

            expect(beforeSendMock).not.toHaveBeenCalled()
            expect(instance.register).not.toHaveBeenCalled()
            expect(console.error).toHaveBeenCalledWith(
                '[Agrid.js]',
                'The string "$agrid_cookieless" was set in agrid.identify which indicates an error. This ID is only used as a sentinel value.'
            )
        })
    })
