...
describe('request', () => {
    ...
    beforeEach(() => {
        ...
        createRequest = (overrides) => ({
            url: 'https://any.agrid-instance.com?ver=1.23.45',
            data: undefined,
            headers: {},
    ...
    describe('xhr', () => {
        ...
        it('performs the request with default params', () => {
            request(
                createRequest({
                    url: 'https://any.agrid-instance.com/',
                    headers: {
                        'x-header': 'value',
                    },
                })
            ...
            expect(mockedXHR.open).toHaveBeenCalledWith(
                'GET',
                'https://any.agrid-instance.com/?_=1700000000000&ver=1.23.45',
                true
            )
    ...
    describe('fetch', () => {
        ...
        it('performs the request with default params', () => {
            ...
            expect(mockedFetch).toHaveBeenCalledWith(
                `https://any.agrid-instance.com?ver=1.23.45&_=1700000000000`,
        ...
        describe('keepalive with fetch and large bodies can cause some browsers to reject network calls', () => {
            it.each([
                ...
                (
                    ...
                    expect(mockedFetch).toHaveBeenCalledWith(
                        `https://any.agrid-instance.com?ver=1.23.45&_=1700000000000${expectedURLParams}`,
                        expect.objectContaining({
                            headers: new Headers(),
        ...
        describe('adding query params to agrid API calls', () => {
            const agridURL = 'https://any.agrid-instance.com/my-url'

            it('extends params in a url', () => {
                const newUrl = extendURLParams(agridURL, {
                    a: true,
                    b: 1,
                    c: 'encoded string 😘',
                })
                expect(newUrl).toEqual(agridURL + '?a=true&b=1&c=encoded%20string%20%F0%9F%98%98')
            })
            ...
            it.each([
                ...
                [
                    ...
                    false,
                    agridURL + '?a=old&b=2&c=encoded%20string%20%F0%9F%98%98',
                ],
                [
                    'replaces multiple existing params when replace=true',
                    agridURL + '?retry_count=1&ver=old',
                    { retry_count: 2, ver: 'new' },
                    true,
                    agridURL + '?retry_count=2&ver=new',
                ],
                [
                ...
                [
                    ...
                    agridURL + '?a=false&b=1&c=encoded%20string%20%F0%9F%98%98',
                    { retry_count: 2, ver: 'new' },
                    true,
                    agridURL + '?a=false&b=1&c=encoded%20string%20%F0%9F%98%98&retry_count=2&ver=new',
                ],
            ])(
                '%s',
        ...
        describe('sendBeacon', () => {
            ...
            it("should encode data to a string and send it as a blob if it's a POST request", async () => {
                ...
                expect(mockedNavigator?.sendBeacon).toHaveBeenCalledWith(
                    'https://any.agrid-instance.com/?_=1700000000000&ver=1.23.45&beacon=1',
                    expect.any(Blob)
                )
            ...
            it('should respect base64 compression', async () => {
                ...
                expect(mockedNavigator?.sendBeacon).toHaveBeenCalledWith(
                    'https://any.agrid-instance.com/?_=1700000000000&ver=1.23.45&compression=base64&beacon=1',
                    expect.any(Blob)
                )
                const blob = mockedNavigator?.sendBeacon.mock.calls[0][1] as Blob
            ...
            it('should respect gzip compression', async () => {
                ...
                expect(mockedNavigator?.sendBeacon).toHaveBeenCalledWith(
                    'https://any.agrid-instance.com/?_=1700000000000&ver=1.23.45&compression=gzip-js&beacon=1',
                    expect.any(Blob)
                )
                const blob = mockedNavigator?.sendBeacon.mock.calls[0][1] as Blob
