...
export class AgridExceptions {
    ...
    sendExceptionEvent(properties: Properties): CaptureResult | undefined {
        ...
        if (this._isExceptionList(exceptionList)) {
            ...
            if (
                !this._instance.config.error_tracking.__captureAgridExceptions &&
                this._isAgridException(exceptionList)
            ) {
                logger.info('Skipping exception capture because it was thrown by the Agrid SDK')
                return
            }
        }
    ...
    private _isAgridException(exceptionList: ErrorTracking.ExceptionList): boolean {
        if (exceptionList.length > 0) {
            const exception = exceptionList[0]
            const frames = exception.stacktrace?.frames ?? []
            const lastFrame = frames[frames.length - 1]
            return lastFrame?.filename?.includes('agrid.com/static') ?? false
        }
