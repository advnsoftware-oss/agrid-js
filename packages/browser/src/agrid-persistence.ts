...
import { PersistentStore, AgridConfig, Properties } from './types'
import {
...
const CASE_INSENSITIVE_PERSISTENCE_TYPES: readonly Lowercase<AgridConfig['persistence']>[] = [
    'cookie',
    'localstorage',
    'localstorage+cookie',
    'sessionstorage',
...
const parseName = (config: AgridConfig): string => {
    let token = ''
    if (config['token']) {
    ...
    if (config['persistence_name']) {
        ...
        return 'ph_' + token + '_agrid'
    }
}

/**
 * Agrid Persistence Object
 * @constructor
 */
export class AgridPersistence {
    private _config: AgridConfig
    props: Properties
    ...
    /**
     * @param {AgridConfig} config initial Agrid configuration
     * @param {boolean=} isDisabled should persistence be disabled (e.g. because of consent management)
     */
    constructor(config: AgridConfig, isDisabled?: boolean) {
        this._config = config
        this.props = {}
    ...
    private _buildStorage(config: AgridConfig) {
        if (
            CASE_INSENSITIVE_PERSISTENCE_TYPES.indexOf(
                config['persistence'].toLowerCase() as Lowercase<AgridConfig['persistence']>
            ) === -1
        ) {
            logger.critical(
        ...
        const storage_type = config['persistence'].toLowerCase() as Lowercase<AgridConfig['persistence']>
    ...
    update_config(config: AgridConfig, oldConfig: AgridConfig, isDisabled?: boolean): void {
