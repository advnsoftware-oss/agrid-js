...
export class AgridSurveys {
    ...
    loadIfEnabled() {
        ...
        const phExtensions = assignableWindow?.__AgridExtensions__
        if (!phExtensions) {
            logger.error('Agrid Extensions not found.')
            return
        }
        ...
        try {
            ...
            if (!loadExternalDependency) {
                ...
                this._handleSurveyLoadError('Agrid loadExternalDependency extension not found.')
                return
            }
    ...
    private _completeSurveyInitialization(
        generateSurveysFn: (instance: Agrid, isSurveysEnabled: boolean) => any,
     ...
     * ### Usage:
     *
     *     agrid.onSurveysLoaded((surveys) => {
     *         // You can work with all surveys
     *         console.log('All available surveys:', surveys)
     *
     *         // Or get active matching surveys
     *         agrid.getActiveMatchingSurveys((activeMatchingSurveys) => {
     *             if (activeMatchingSurveys.length > 0) {
     *                 agrid.renderSurvey(activeMatchingSurveys[0].id, '#survey-container')
     *             }
     *         })
     *     })
     *
