...
export class TracingHeaders {
    ...
    private _loadScript(cb: () => void): void {
        if (assignableWindow.__AgridExtensions__?.tracingHeadersPatchFns) {
            // already loaded
            cb()
        }

        assignableWindow.__AgridExtensions__?.loadExternalDependency?.(this._instance, 'tracing-headers', (err) => {
            if (err) {
    ...
    private _startCapturing = () => {
        if (isUndefined(this._restoreXHRPatch)) {
            assignableWindow.__AgridExtensions__?.tracingHeadersPatchFns?._patchXHR(
        ...
        if (isUndefined(this._restoreFetchPatch)) {
            assignableWindow.__AgridExtensions__?.tracingHeadersPatchFns?._patchFetch(
                this._instance.config.__add_tracing_headers || [],
                this._instance.get_distinct_id(),
                this._instance.sessionManager
            )
