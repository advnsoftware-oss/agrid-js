...
const logger = createLogger('[Agrid ExternalIntegrations]')
...
export class ExternalIntegrations {
    ...
    private _loadScript(name: ExternalExtensionKind, cb: () => void): void {
        assignableWindow.__AgridExtensions__?.loadExternalDependency?.(this._instance, name, (err) => {
            if (err) {
    ...
    public startIfEnabledOrStop() {
        for (const [key, value] of Object.entries(this._instance.config.integrations ?? {})) {
            ...
            if (value && !assignableWindow.__AgridExtensions__?.integrations?.[key as ExternalIntegrationKind]) {
                this._loadScript(MAPPED_INTEGRATIONS[key as ExternalIntegrationKind], () => {
                    assignableWindow.__AgridExtensions__?.integrations?.[key as ExternalIntegrationKind]?.start(
                        this._instance
                    )
                })
            }
            ...
            if (!value && assignableWindow.__AgridExtensions__?.integrations?.[key as ExternalIntegrationKind]) {
                assignableWindow.__AgridExtensions__?.integrations?.[key as ExternalIntegrationKind]?.stop()
            }
        }
    }
}
